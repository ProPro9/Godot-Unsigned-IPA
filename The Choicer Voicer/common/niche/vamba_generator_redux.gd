class_name VambaGeneratorRedux extends Resource


func generate_uniform_shuffle(input: OmniClipCollection, size: int) -> PackedStringArray:
	...
	var backup_bucket: Array[String] = []
	while !total.is_empty():
		var checking: OmniClip = total.pop_back()
		if !checking.has_been_seen(): output.append(checking.seen_path)
		else:
			if randf() <= unseen_percentage: backup_bucket.append(checking.seen_path)
			else: output.append(checking.seen_path)
		if output.size() == size: return output
	while !backup_bucket.is_empty() and (output.size() < size): output.append(backup_bucket.pop_back())
	return output
func generate_packweighted_shuffle(input: OmniClipCollection, size: int) -> PackedStringArray:
	var output: PackedStringArray = []
	var unseen_percentage: float = float(Profile.unseen_percent) / 100.0
...
func __recursive_pseudo(dict: Dictionary, input: Array, clip: OmniClip) -> void :
	...
	if input.size() > 1:
		...
		dict[key]["/"].append(clip.seen_path)
