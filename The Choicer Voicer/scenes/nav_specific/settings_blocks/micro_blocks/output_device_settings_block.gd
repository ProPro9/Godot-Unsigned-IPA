...
func _setup() -> void :
	...
	for index: int in device_list.size():
		...
		if device_name == Profile.audio_device_out: temp_disable_select_trigger = true;option_button.select(index);temp_disable_select_trigger = false
func _device_selected(_index: int) -> void :
	...
	Profile.audio_device_out = writeable_name
